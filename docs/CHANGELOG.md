...
- [新功能] 新增外部持仓截图快照 API：支持上传同花顺股票持仓 / 支付宝基金持仓截图，生成 draft 快照、人工确认后落库，并按需同步飞书云文档；该能力与原 `portfolio` 交易账本隔离，避免用截图快照污染交易流水。
- [改进] 新增外部持仓截图提醒配置项：支持单独开启截图快照能力、配置同花顺 / 支付宝提醒时间、提醒渠道和飞书文档同步开关，并同步更新配置注册表与环境变量模板。
- [chore] 移除主项目内临时 `docker/ocr-service/` 脚手架，避免与独立资产截图解析服务重复维护。
- [文档] 将完整指南中的共享 OCR 脚手架说明更新为独立资产截图解析服务接入说明。
